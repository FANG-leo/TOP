  ...
  for (size_t i = 0; i < mesh->width; i++) {
    for (size_t j = 0; j < mesh->height; j++) {
      for (size_t k = 0; k < DIRECTIONS; k++) {
        Mesh_set_value(mesh, i, j, k, equil_weight[k]);
      }
    }
  }
}
...
void setup_init_state_global_poiseuille_profile(Mesh* mesh, lbm_mesh_type_t* mesh_type, const lbm_comm_t* mesh_comm) {
  ...
  for (size_t i = 0; i < mesh->width; i++) {
    for (size_t j = 0; j < mesh->height; j++) {
      ...
      for (size_t k = 0; k < DIRECTIONS; k++) {
        Mesh_set_value(mesh, i, j, k, compute_equilibrium_profile(v, rho, k));
...
void setup_init_state_border(Mesh* mesh, lbm_mesh_type_t* mesh_type, const lbm_comm_t* mesh_comm) {
  ...
  if (mesh_comm->top_id == -1) {
    for (size_t i = 0; i < mesh->width; i++) {
      for (size_t k = 0; k < DIRECTIONS; k++) {
        // Compute equilibrium.
        Mesh_set_value(mesh, i, 0, k, compute_equilibrium_profile(v, rho, k));
        // Mark as bounce back
  ...
  if (mesh_comm->bottom_id == -1) {
    for (size_t i = 0; i < mesh->width; i++) {
      for (size_t k = 0; k < DIRECTIONS; k++) {
        // Compute equilibrium.
        Mesh_set_value(mesh, i, mesh->height - 1, k, compute_equilibrium_profile(v, rho, k));
        // Mark as bounce back
