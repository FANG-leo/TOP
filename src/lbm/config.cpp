...
static bool is_output_disabled_value(const char* value) {
  return strcmp(value, "none") == 0 || strcmp(value, "null") == 0 || strcmp(value, "off") == 0;
}
...
void setup_default_values(void) {
  ...
  lbm_gbl_config.show_progress   = 1;
}
...
void load_config(const char* filename) {
  ...
  while (fgets(buffer, 1024, fp) != NULL) {
    ...
    } else if (sscanf(buffer, "show_progress = %d\n", &intValue) == 1) {
      lbm_gbl_config.show_progress = (intValue != 0);
    } else if (sscanf(buffer, "output_filename = %s\n", buffer2) == 1) {
      if (is_output_disabled_value(buffer2)) {
...
void config_cleanup(void) {
  if (lbm_gbl_config.output_filename != NULL && lbm_gbl_config.output_filename != default_output_filename) {
    free((void*)lbm_gbl_config.output_filename);
  }
}

void print_config(void) {
  const char* output_filename = lbm_gbl_config.output_filename == NULL ? "(disabled)" : lbm_gbl_config.output_filename;
